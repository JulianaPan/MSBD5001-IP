...
features = data.drop('id', axis=1).drop('playtime_forever', axis=1)
training_features, testing_features, training_target, testing_target = train_test_split(features, data['playtime_forever'], test_size=0.33, random_state=None)
...
joblib.dump(exported_pipeline, "model/Model.m")
...
# predict
df_test = pd.read_csv('data/new2_test.csv')
X_test = df_test.loc[:,df_test.columns!='id']
y_pred = exported_pipeline.predict(X_test)
df_tpot20 = pd.DataFrame()
df_tpot20['id'] = df_test.loc[:,'id']
df_tpot20['playtime_forever'] = y_pred
df_tpot20.to_csv("result/result.csv",index=False)